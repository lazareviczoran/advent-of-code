...
fn find_sum_after_20th_gen(pots: &BTreeMap<i32, char>, patterns: &BTreeMap<String, char>) -> i32 {
    let mut curr_pots = pots.clone();
...
fn apply_transformations(pots: &mut BTreeMap<i32, char>, patterns: &BTreeMap<String, char>) {
...
fn read_input(filename: &str) -> (BTreeMap<i32, char>, BTreeMap<String, char>) {
    ...
    let mut rows: Vec<String> = read_to_string_in_module!(filename)
        .split_terminator('\n')
        .map(|s| s.to_string())
        .collect();
    let initial_state_str = rows.remove(0);
    let initial_state: Vec<&str> = initial_state_str.split_terminator(": ").collect();
    rows.remove(0);
    for (i, ch) in initial_state[1].chars().enumerate() {
        pots.insert(i as i32, ch);
    }
