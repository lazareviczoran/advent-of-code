...
fn find_max_door_pass(map: &mut Vec<Vec<char>>, start_pos: &(usize, usize)) -> Vec<usize> {
    ...
    while !queue.is_empty() {
        ...
        if curr_x + 2 < map.len() && map[curr_x + 1][curr_y] == '|' && !visited[curr_x + 2][curr_y]
        {
            queue.push((curr_x + 2, curr_y, curr_dist + 1));
        }
        if curr_x >= 2 && map[curr_x - 1][curr_y] == '|' && !visited[curr_x - 2][curr_y] {
            queue.push((curr_x - 2, curr_y, curr_dist + 1));
        }
        if curr_y + 2 < map.len() && map[curr_x][curr_y + 1] == '-' && !visited[curr_x][curr_y + 2]
        {
            queue.push((curr_x, curr_y + 2, curr_dist + 1));
        }
        if curr_y >= 2 && map[curr_x][curr_y - 1] == '-' && !visited[curr_x][curr_y - 2] {
            queue.push((curr_x, curr_y - 2, curr_dist + 1));
        }
    }
    distances
