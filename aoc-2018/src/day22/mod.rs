...
fn calculate_risk_level(map: &HashMap<(i32, i32), FieldInfo>, target: &(i32, i32)) -> i32 {
...
fn build_map(depth: i32, target: &(i32, i32)) -> HashMap<(i32, i32), FieldInfo> {
    ...
    pub fn new(
        ...
        let geo_index = if pos == *target || pos == (0, 0) {
            ...
            if let Some(field) = map.get(&(pos.0 - 1, pos.1)) {
                ...
                val1 = FieldInfo::new(map, target, (pos.0 - 1, pos.1), depth).erosion_level;
            }
            let val2;
            if let Some(field) = map.get(&(pos.0, pos.1 - 1)) {
                val2 = field.erosion_level;
            } else {
                val2 = FieldInfo::new(map, target, (pos.0, pos.1 - 1), depth).erosion_level;
            }
            val1 * val2
        };
