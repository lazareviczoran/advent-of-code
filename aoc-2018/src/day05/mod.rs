...
pub fn run() {
    let polymer = read_to_string_in_module!("input.txt");
...
fn find_shortest_polymer_by_removing_one_item(polymer: &str) -> String {
    ...
    for ch in polymer.chars() {
        ...
        if !used_elements.contains(&lower) {
            let mut polymer_candidate = polymer.to_string();
...
mod test {
    ...
    #[test]
    fn part2_test() {
        let shortest_polymer = find_shortest_polymer_by_removing_one_item("dabAcCaCBAcCcaDA");
