...
fn find_strategy2_solution(sleep_time_intervals: &HashMap<usize, Vec<(usize, usize)>>) -> usize {
    let mut guard_id = 0;
...
fn find_strategy1_solution(sleep_time_intervals: &HashMap<usize, Vec<(usize, usize)>>) -> usize {
    let mut guard_id = 0;
    let mut current_max = 0;
    ...
    let (minute, _) =
        find_most_frequent_minute_for_sleep(sleep_time_intervals.get(&guard_id).unwrap());

    guard_id * minute
}

fn find_most_frequent_minute_for_sleep(intervals: &Vec<(usize, usize)>) -> (usize, usize) {
...
fn read_input(filename: &str) -> HashMap<usize, Vec<(usize, usize)>> {
    let mut items: Vec<String> = read_to_string_in_module!(filename)
        .split_terminator('\n')
        .map(|a| a.to_string())
        .collect();
    items.sort();
    ...
    let mut sleep_time_intervals: HashMap<usize, Vec<(usize, usize)>> = HashMap::new();
    ...
    while let Some(guard) = iter.next() {
        ...
        while next_line.is_some() && !next_line.unwrap().contains("Guard") {
            ...
            if let Some(guard_sleep_intervals) = sleep_time_intervals.get_mut(&guard_id) {
