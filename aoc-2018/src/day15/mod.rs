...
fn search(
    ...
    if high == low {
        ...
        search(arena, high, high * 2, elves_count, computed)
    } else if high_count < elves_count {
        search(arena, high, high * 2, elves_count, computed)
    } else if low_count < elves_count {
        search(arena, mid, high, elves_count, computed)
    } else {
        search(arena, low - (high - low) / 2, low, elves_count, computed)
    }
}
...
impl CombatArena {
    ...
    pub fn simulate_combat(&mut self) -> usize {
        ...
        let health_sum = self.units.iter().fold(0, |acc, u| acc + u.health as usize);
    ...
    pub fn count_elves(&self) -> usize {
        self.units
            .iter()
            .fold(0, |acc, u| if u.unit_type == 'E' { acc + 1 } else { acc })
    }
}
...
impl Unit {
    ...
    pub fn get_opponent_candidates(&self, combat_arena: &CombatArena) -> Vec<Unit> {
