...
fn shortest_path_for_all_starts(map: &[Vec<Node>], target: (usize, usize)) -> Option<usize> {
...
fn shortest_path(map: &[Vec<Node>], start: (usize, usize), goal: (usize, usize)) -> Option<usize> {
...
fn parse(filename: &str) -> HillMap {
    let map = utils::read_to_string_in_module!(filename)
        .lines()
        .map(|line| {
...
#[cfg(test)]
mod tests {
    use super::{parse, shortest_path, shortest_path_for_all_starts};
