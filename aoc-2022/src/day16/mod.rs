    ...
    if let Some((value, neighbours)) = valves.get(&curr_valve) {
        if *value > 0 && !used.contains(&curr_valve) {
            max_sum = max_sum.max(calc_max_pressure_release_rec(
                ...
                remaining_time - 1,
                curr_valve.clone(),
                &used | &BTreeSet::from_iter(vec![curr_valve.clone()]),
                cache,
            ));
        }
        max_sum = max_sum.max(
...
type Map = HashMap<Rc<String>, usize>;

#[allow(dead_code)]
fn get_distance_between_valued_nodes(
    valves: &HashMap<Rc<String>, (usize, Vec<Rc<String>>)>,
) -> HashMap<Rc<String>, (usize, Map)> {
    let mut dist: HashMap<Rc<String>, (usize, Map)> = HashMap::new();
