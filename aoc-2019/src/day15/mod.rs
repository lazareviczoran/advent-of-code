        ...
        for move_cmd in [1, 2, 3, 4] {
            ...
            if visited.get(&(curr_pos.x, curr_pos.y)).is_none() {
                ...
                for i in 0..new_commands.len() {
                    let (output, _, _, _, _) =
                        compute(&mut memory_clone, &[new_commands[i]], 0, 0, 0);
...
fn search_oxygen(
    ...
    let (output, _, _, _, _) = compute(memory, &[move_cmd], 0, 0, 0);
